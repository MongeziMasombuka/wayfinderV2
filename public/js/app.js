...
async function loadAll() {
  try {
    const [storesRes, catsRes, wingsRes, ameRes, evRes] = await Promise.all([
      fetch("/api/stores"),
    ...
    document.getElementById("all-stores").innerHTML =
      '<div class="dir-empty">❌ Failed to load. Make sure backend is running and refresh after a few seconds.</div>';
    document.getElementById("events-area").innerHTML =
      '<div class="event-card">Unable to load events</div>';
    document.getElementById("amenities-area").innerHTML =
      '<div class="amenity-pill">Unable to load amenities</div>';
  }
}
...
function renderEvents() {
  const c = document.getElementById("events-area");
  if (!events.length) {
    c.innerHTML = '<div class="event-card">No events today</div>';
    return;
  }
  c.innerHTML = events
    .map(
      (e) =>
        `<div class="event-card"><div class="event-title">📢 ${escapeHtml(e.title)}</div><div style="font-size:12px;">${escapeHtml(e.description)}</div><div style="font-size:10px;margin-top:6px;">${e.store_name ? escapeHtml(e.store_name) : e.wing_name ? escapeHtml(e.wing_name) : "Grand Atrium"}</div></div>`,
    )
    .join("");
}
function renderAmenities() {
  const c = document.getElementById("amenities-area");
  c.innerHTML = amenities
    .map(
      (a) =>
        `<div class="amenity-pill">${a.icon || "📍"} ${escapeHtml(a.name)} · ${escapeHtml(a.wing_name || "")} L${a.level_id}</div>`,
    )
    .join("");
}
function renderFilterBar() {
  ...
  document.getElementById("filter-bar").innerHTML = filters
    .map(
      (f) =>
        `<button class="fpill${activeFilter === String(f.id) ? " on" : ""}" data-filter="${f.id}" onclick="setFilter('${f.id}')">${f.label}</button>`,
    )
    .join("");
}
...
function renderDirectory() {
  ...
  let filtered = stores;
  if (activeFilter !== "all")
    filtered = stores.filter((s) => s.category_id == activeFilter);
  const qrIcon = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="11" height="11"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="15" y="15" width="2" height="2"/><rect x="19" y="15" width="2" height="2"/><rect x="15" y="19" width="2" height="2"/><rect x="19" y="19" width="2" height="2"/></svg>`;
  document.getElementById("all-stores").innerHTML = filtered
    .map(
      (s) =>
        `<div class="dir-row"><div class="dir-icon" style="background:${s.ibg}">${s.emoji}</div><div class="dir-info"><div class="dir-name">${escapeHtml(s.name)}</div><div class="dir-meta"><span class="chip chip-lv">L${s.level_id}</span><span class="chip ${s.color_class}">${escapeHtml(s.category_name)}</span><span class="chip chip-id">${escapeHtml(s.id)}</span></div><div class="s-wing">${escapeHtml(s.wing_name)}</div></div><div class="dir-btns"><button class="qr-btn" onclick="showQrModal('${s.id}')" title="QR Code">${qrIcon} QR</button><button class="set-btn" onclick="setFromDirectory('${s.id}')">Navigate</button></div></div>`,
    )
    .join("");
}
function setFromDirectory(id) {
  setField("from", id);
  openSheet("to");
...
function navigateFromQrModal() {
  if (qrModalStoreId) {
    closeQrModal();
    setField("from", qrModalStoreId);
    setTimeout(() => openSheet("to"), 150);
  }
}
...
// Prefer the rear camera; fall back to any camera if the device has no
// rear-facing one (e.g. laptops), instead of failing outright.
